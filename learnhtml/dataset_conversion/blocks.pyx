...
from learnhtml.compat import str_list_cast, str_dict_cast, str_block_cast, str_block_list_cast, str_cast, bytes_cast
...
def simple_tokenizer(x):
    x = str_cast(x)
    return [ele.lower() for ele in re_tokenizer.split(x) if len(ele) > 0]
